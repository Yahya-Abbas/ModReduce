...
class VGG(nn.Module):
    ...
    def __init__(self, features, split=None, num_classes=1000, init_weights=True):
    ...
    def forward(self, x, is_feat=False):
        hidden_layers = []
        for left, right in zip(self.split, self.split[1:]):
...
def make_layers(cfg, split, batch_norm=False):
    layers = []
    in_channels = 3
    for v in cfg:
        if v == 'M':
...
def _vgg(arch, cfg, batch_norm, pretrained, progress, **kwargs):
    ...
    model = VGG(make_layers(cfgs[cfg], splits[cfg], batch_norm=batch_norm), split=splits[cfg], **kwargs)
    if pretrained:
